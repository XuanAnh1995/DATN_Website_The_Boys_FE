...
const ColorService = {
    ...
    getAllColors: async (search = "", page = 0, size = 10, sortBy = "id", sortDir = "asc") => {
        try {
            ...
            console.error("❌ Lỗi khi lấy danh sách màu sắc:", error.response?.data || error.message);
    ...
    getColorById: async (id) => {
        try {
            ...
            console.error(`❌ Lỗi khi lấy màu sắc ${id}:`, error.response?.data || error.message);
            throw error;
        }
    },
    ...
    createColor: async (colorData) => {
        try {
            ...
            console.error("❌ Lỗi khi tạo màu sắc:", error.response?.data || error.message);
            throw error;
        }
    },
    ...
    updateColor: async (id, colorData) => {
        try {
            ...
            console.error(`❌ Lỗi khi cập nhật màu sắc ${id}:`, error.response?.data || error.message);
            throw error;
        }
    },
    ...
    toggleStatusColor: async (id) => {
        try {
            ...
            console.error(`❌ Lỗi khi thay đổi trạng thái màu sắc ${id}:`, error.response?.data || error.message);
            throw error;
        }
    },
};
