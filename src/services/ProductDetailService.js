...
const ProductDetailService = {
    ...
    async createProductDetail(productDetailData) {
        try {
            const response = await axios.post(`http://localhost:8080/api/product-details`, productDetailData,
                {
    ...
    async generateProductDetails(generateModel) {
        try {
            const response = await axios.post(`http://localhost:8080/api/product-details/generate`, generateModel,
                {
                    headers: { 'Content-Type': 'application/json' }
                });
            return response.data.data;
    ...
    async getAllProductDetails(params) {
        try {
            ...
            const response = await axios.get(`http://localhost:8080/api/product-details`, { params: formattedParams });
    ...
    async updateProductDetail(id, updateData) {
        try {
            const response = await axios.put(`http://localhost:8080/api/product-details/${id}`, updateData,
                {
                    headers: { 'Content-Type': 'application/json' }
                });
            return response.data.data;
