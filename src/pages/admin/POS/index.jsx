    ...
    return (
        <div className="p-4 bg-gray-100 min-h-screen">
            ...
            <div className="flex items-center bg-red-600 p-3 text-white">
                ...
                />
                <button className="ml-2 bg-white text-red-600 px-4 py-2 rounded">Hóa đơn mặc định</button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
                ...
                <div className="bg-white p-4 rounded shadow">
                    ...
                    <select value={selectedCustomer} onChange={(e) => setSelectedCustomer(e.target.value)} className="border p-1 w-full">
                        <option value="">Chọn khách hàng</option>
                        {customers.map((c) => (
                            <option key={c.id} value={c.id}>{c.customerName}</option>
                        ))}
                    </select>
                    <button className="bg-blue-600 text-white w-full py-2 mt-4 rounded"> + </button>
                    ...
                    <p>Chiết khấu (F6): {discount.toLocaleString()} VND</p>
                    <p>KHÁCH PHẢI TRẢ: {(totalAmount - discount).toLocaleString()} VND</p>
                    <p>Tiền khách đưa (F8):
                        <input type="number" value={customerPaid} onChange={(e) => setCustomerPaid(Number(e.target.value))} className="ml-2 border p-1" />
                    </p>
                    <p>Tiền thừa trả khách: {changeAmount.toLocaleString()} VND</p>
                    <div className="mt-2">
                        <label className="mr-2">Phương thức thanh toán:</label>
                        <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="border p-1">
                            <option value="cash">Tiền mặt</option>
                            <option value="card">Thẻ</option>
                            <option value="bank">Chuyển khoản</option>
                        </select>
                    </div>
                    <button className="bg-green-600 text-white w-full py-2 mt-4 rounded">Thanh toán (F9)</button>
                </div>
            </div>
        </div>
    );
